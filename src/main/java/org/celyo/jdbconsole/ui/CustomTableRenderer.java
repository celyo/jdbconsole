...
public class CustomTableRenderer<V> implements TableRenderer<V> {
  ...
  /**
   * Sets the style to be used when separating the table header row from the actual "data" cells
   * below. This will cause a new line to be added under the header labels, unless set to
   * {@code TableCellBorderStyle.None}.
   *
  ...
  /**
   * Sets the style to be used when separating the table header labels from each other. This will
   * cause a new column to be added in between each label, unless set to
   * {@code TableCellBorderStyle.None}.
   *
  ...
  /**
   * Sets the style to be used when vertically separating table cells from each other. This will
   * cause a new line to be added between every row, unless set to
   * {@code TableCellBorderStyle.None}.
   *
  ...
  /**
   * Sets the style to be used when horizontally separating table cells from each other. This will
   * cause a new column to be added between every row, unless set to
   * {@code TableCellBorderStyle.None}.
   *
  ...
  public TerminalSize getPreferredSize(Table<V> table) {
    ...
    int visibleColumns = calcVisibleColumns(table, false);
    int visibleRows = calcVisibleRows(table, false);
    int viewLeftColumn = calcLeftColumn(table, visibleColumns);
    int viewTopRow = calcTopRow(table, visibleRows);
    ...
    if (rows.size() == 0) {
      for (int columnIndex = viewLeftColumn; columnIndex < viewLeftColumn + visibleColumns; columnIndex++) {
        ...
        } else if (columnSizes.get(listOffset) < columnSize) {
    ...
    for (int columnIndex = 0; columnIndex < columnHeaders.size(); columnIndex++) {
      for (int rowIndex = viewTopRow; rowIndex < Math.min(rows.size(), viewTopRow + visibleRows); rowIndex++) {
        ...
        } else if (rowSizes.get(listOffset) < rowSize) {
  ...
  private int drawHeader(TextGUIGraphics graphics, Table<V> table) {
    ...
    int visibleColumns = calcVisibleColumns(table, true);
    int viewLeftColumn = calcLeftColumn(table, visibleColumns);
    int topPosition = 0;
    int leftPosition = 0;
  ...
  private void drawRows(TextGUIGraphics graphics, Table<V> table, int topPosition) {
    ...
    TableModel<V> tableModel = table.getTableModel();
    List<List<V>> rows = tableModel.getRows();
    int visibleRows = calcVisibleRows(table, true);
    int visibleColumns = calcVisibleColumns(table, true);
    int viewTopRow = calcTopRow(table, visibleRows);
    int viewLeftColumn = calcLeftColumn(table, visibleColumns);
  ...
  private char getJunctionCharacter(TableCellBorderStyle mainStyle, TableCellBorderStyle styleAbove, TableCellBorderStyle styleBelow) {
    if (mainStyle == TableCellBorderStyle.SingleLine) {
      ...
      } else if (styleBelow == TableCellBorderStyle.SingleLine) {
  ...
  int calcVisibleColumns(Table<V> table, boolean useTableSize) {
    int visibleColumns = table.getVisibleColumns();
    if (visibleColumns == 0) {
      if (useTableSize && !columnSizes.isEmpty()) {
        int tableSize = table.getSize().getColumns() - 1;
        int totalSize = 0;
  ...
  int calcVisibleRows(Table<V> table, boolean useTableSize) {
    int visibleRows = table.getVisibleRows();
    if (visibleRows == 0) {
      if (useTableSize && !rowSizes.isEmpty()) {
        int tableSize = table.getSize().getRows() - headerSizeInRows - 1;
        int totalSize = 0;
  ...
  int calcLeftColumn(Table<V> table, int visibleColumns) {
    int viewLeftColumn = table.getViewLeftColumn();
    
    if (visibleColumns > 0) {
      if (table.getSelectedColumn() < viewLeftColumn) {
        viewLeftColumn = table.getSelectedColumn();
      } else if (table.getSelectedColumn() >= viewLeftColumn + visibleColumns) {
        viewLeftColumn = Math.max(0, table.getSelectedColumn() - visibleColumns + 1);
      }
    }
  ...
  int calcTopRow(Table<V> table, int visibleRows) {
    int viewTopRow = table.getViewTopRow();
    
    if (visibleRows > 0) {
      if (table.getSelectedRow() < viewTopRow) {
        viewTopRow = table.getSelectedRow();
      } else if (table.getSelectedRow() >= viewTopRow + visibleRows) {
        viewTopRow = Math.max(0, table.getSelectedRow() - visibleRows + 1);
      }
    }
